...
describe('User is on the Registration window', function () {
  ...
  it('Clicking Settings button lands user to Registration window', function () {
    ...
    return app.client.waitUntilWindowLoaded()
      ...
      .then(function () {
        return app.client.waitUntilWindowLoaded()
          .element("//span[contains(text(),' Existing Registrations ')]")
          .getText().should.eventually.equal('Existing Registrations')
      })
  })
...
it('Closing the New Registration form lands to Registration window',function(){
  ...
  .then(function(){
  return app.client.waitUntilWindowLoaded()
  .element("//strong[contains(text(),'SETTINGS')]")
  .getText().should.eventually.equal('SETTINGS')
 })
})
})
...
it.only('Serial number should autopopulated and user should not be able to edit Serial number field', function(){
  var e = new Date().getTime() + (3000);
  while (new Date().getTime() <= e) { }
  return app.client.waitUntilWindowLoaded().click("//main[@class='q-page']//button")
  .then(function () {
